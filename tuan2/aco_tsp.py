...
SCENT_MIN = 0.2 # mùi khởi tạo
ALPHA = 1
BETA = 3
EVAP_COEFF = 0.6 # tham số bay hơi 
QI = 150
...
def build_graph(filename):
    ...
    print(f'Number of cities :{length}')
...
def update_scents(ants, scent, distance):
    ...
    for key, value in path_travelers.items():
        src = key[0]
        dst = key[1]
        updated_scent  = (1 - EVAP_COEFF) * scent[src][dst] + QI * value * (1/distance[src][dst])
...
def place_ants_randomly(num_ants, num_cities):
    ...
    for i in range(num_ants):
        ants[i] = Ant(np.random.randint(0, num_cities - 1))
    return ants
...
def shortest_path(ants, min_dist, min_path, distance):
    ...
    for ant in ants:
        #print ant.path
        dist = 0
        for i in range(ant.path.shape[0] - 1):
            src = ant.path[i]
            dst = ant.path[i + 1]
            dist = dist + distance[src][dst]
        if dist < min_dist:
...
def two_opt_swap(route, i, k):
    ...
    for a in reversed(range(i, k+1)):
        new_route[j] = route[a]
        j = j + 1

    for a in range(k+1, route_len):
        new_route[j] = route[a]        
        j += 1
...
def two_opt_iter(curr_path, distance):
    ...
    for i in range(1, num_nodes - 1):
        for k in range(i + 1, num_nodes - 1):
...
def main():
    ...
    parser.add_argument('-a', action='store',dest='num_ants',default=80, type=int)
    parser.add_argument('-i', action='store',dest='iterations',default=20, type=int)
    parser.add_argument('-f', action='store',dest='file',default='D:/C/uet_4_2/rbpt/tuan2/qatar.txt',type=str)
    ...
    print("Time process = %f" %elapsed_time)
    print ("Minimum distance found = %f" % min_dist)

    # plot(nodes,min_psath)

    with open ('D:/C/uet_4_2/rbpt/tuan2/result.txt', "a") as result:
        # result.write("SCENT_MIN, BETA, ALPHA, EVAP_COEFF, num_ants, iterations, min_dist, elapsed_time\n")
        result.write(str(SCENT_MIN)+","+str(BETA)+","+str(ALPHA)+","+str(EVAP_COEFF)+","+
                     str(num_ants)+","+str(iterations)+","+str(min_dist)+","+str(elapsed_time)+ "\n")
